...
import { TrendingUp, Loader2, LogOut, Star, MapPin, Award, BookOpen, Clock, ChevronRight, Flame, Users, Briefcase } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const milestones = userData ? [
    ...
    { id: 2, title: userData.interests?.[0] || "Foundation Course", status: userData.progress && userData.progress >= 30 ? "completed" : userData.progress && userData.progress >= 10 ? "current" : "upcoming", description: "Foundation learning" },
    { id: 3, title: "Skill Certification", status: userData.progress && userData.progress >= 50 ? "completed" : userData.progress && userData.progress >= 30 ? "current" : "upcoming", description: "Industry certification" },
    { id: 4, title: "Practical Application", status: userData.progress && userData.progress >= 70 ? "completed" : userData.progress && userData.progress >= 50 ? "current" : "upcoming", description: "Real-world projects" },
    { id: 5, title: "Career Ready", status: userData.progress && userData.progress >= 90 ? "completed" : userData.progress && userData.progress >= 70 ? "current" : "upcoming", description: "Start your career" }
  ] : [];
                    ...
                    {milestones.map((milestone, index) => (
                      <div key={milestone.id} className="flex items-start gap-3 min-w-0">
                        <div className={`flex flex-col items-center ${milestone.status === "completed" ? "text-primary" :
                          ...
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 ${milestone.status === "completed" ? "bg-primary border-primary text-white" :
                ...
                <CardContent className="space-y-3">
                  <div className="flex items-center justify-between gap-3 p-3 bg-green-50 dark:bg-green-950/30 rounded-lg min-w-0">
                    <div className="min-w-0">
                      ...
                      <p className="text-sm text-green-600 dark:text-green-400 truncate">{userData?.target_roles || userData?.career_goal || userData?.interests?.[0] || 'Technology'}</p>
                    </div>
