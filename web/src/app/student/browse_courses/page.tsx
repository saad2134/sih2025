...
export default function BrowseCourses() {
    ...
    const [bookmarkedCourses, setBookmarkedCourses] = React.useState<string[]>([]);
    ...
    const fetchCourses = async () => {
        try {
            setLoading(true);
            const topic = selectedCategory === "All" ? (searchQuery || undefined) : selectedCategory;
            const difficulty = selectedLevel === "All Levels" ? undefined : selectedLevel.toLowerCase();
            ...
            if (res.success && res.data) {
                const items = ((res.data as any).items || []).map((c: any) => ({
                    ...
                    image: "/api/placeholder/400/200",
                    tags: c.style_tags || [],
                    match: Math.round((c.vark_v_score || 0.25) * 100) || 85,
                    completionRate: Math.round(c.completion_rate) || 75,
                    url: c.url || '#'
                }));
                setCoursesList(items);
                setTotalCourses((res.data as any).total || items.length);
            }
            
            const savedRes = await apiService.getSavedCourses();
            if (savedRes.success && savedRes.data) {
                setBookmarkedCourses(savedRes.data.map(item => String(item.id)));
            }
        } catch (err) {
    ...
    React.useEffect(() => {
        fetchCourses();
    }, [selectedCategory, selectedLevel, currentPage, searchQuery]);
    ...
    const filteredCourses = coursesList.filter(course => {
        ...
        return matchesDuration && matchesPriceType;
    }).sort((a, b) => {
        switch (sortBy) {
                                        ...
                                        <Card className="h-full flex flex-col hover:shadow-lg transition-shadow w-full">
                                            ...
                                            <CardContent className="flex-1 space-y-2 sm:space-y-3 px-3 sm:px-4">
                                             ...
                                             <CardFooter className="pt-0 px-3 sm:px-4 pb-3 sm:pb-4 flex gap-2">
                                                 <Button className="flex-1 text-xs sm:text-sm" onClick={() => handleEnrol(course.id)}>
                                                     Enroll Now
